...
DyWebSocketHandler.handleMessage()
    │
    ├─→ [第一步：权限验证]
    ...
    │
    ├─→ [第二步：数据库查询音乐]
    │   MusicLibraryRepository.findByName()
    │   → 查询本地音乐库
    │   ✓ 找到音乐文件
    │
    ├─→ [第三步：检查缓存]
    │   IntroCacheRepository.findByName()
    │   ├─ 缓存命中 (80% 概率)
    │   │  → 直接返回缓存的口播文案
    │   │  → 跳到第六步
    │   │
    ...
    │
    ├─→ [第四步：调用 AI 生成口播]
    │   GeminiService.generateIntroText()
    │   (或其他配置的大模型)
    │   │
    │   └─ 发送 HTTP 请求到 Gemini API
    │      {
    │        "prompt": "为黄龄的算什么男人为小王生成30字口播",
    ...
    │
    ├─→ [第六步：文本清理和处理]
    │   TextCleaningService.clean()
    │   → 移除特殊字符
    │   → 格式规范化
    │
    ├─→ [第七步：TTS 文本转语音]
    │   TtsService.synthesizeAndSave()
    │   (使用 Edge TTS)
    │   │
    │   └─ 生成音频文件
    │      /tts_output/song_算什么男人_20240101.mp3
    │
    └─→ [第八步：返回给前端并播放]
       返回响应：
       {
